...
# =============================================================================
# Terminal-Bench Mode Support
# =============================================================================

TBENCH_SYSTEM_PROMPT = """You are an expert terminal user solving a task. You have access to a bash terminal.
...
class TmuxBridge:
    ...
    def is_busy(self, timeout: float = 0.5) -> bool:
        ...
        # Check if there's a running process in the pane
        ...
        # If the pane is just running bash/zsh/sh, it's not busy
    ...
    def execute_command(self, command: str, timeout: int = 60) -> str:
        ...
        # Capture current output length to detect new output
        ...
        # Return the difference (new output)
        # Simple approach: return everything after the command
        if command in after:
...
class TerminalBenchRunner:
    ...
    def _call_llm(self, messages: List[Dict[str, str]]) -> str:
        ...
        if self.provider == "anthropic":
            ...
            full_messages = [{"role": "system", "content": TBENCH_SYSTEM_PROMPT}] + messages
...
def run_terminal_bench_mode(args) -> int:
    ...
    # Get tmux session ID from environment
    ...
    # Determine provider from model or environment
    model = os.environ.get("TODOFORAI_MODEL", args.model or "claude-sonnet-4-5")
    ...
    # Output token counts for adapter to parse
